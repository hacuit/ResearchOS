...
export function NotePanel({ ideaId }: { ideaId: string }) {
  ...
  async function loadNotes() {
    try {
      const res = await fetch(`${API_BASE}/ideas/${ideaId}/update_logs?limit=50`, { headers });
      if (res.ok) {
  ...
  async function saveNote() {
    ...
    try {
      const res = await fetch(`${API_BASE}/ideas/${ideaId}/update_logs`, {
        method: "POST",
        headers,
        body: JSON.stringify({
          source: "note",
  ...
  async function deleteNote(noteId: string) {
    try {
      const res = await fetch(`${API_BASE}/update_logs/${noteId}`, { method: "DELETE", headers });
