...
export default function Home() {
  ...
  async function loadDashboard() {
    const res = await fetchRetry(`${API_BASE}/dashboard/overview?month=${month}`, { headers });
    if (!res.ok) return;
  ...
  async function loadTasks(ideaId?: string) {
    ...
    if (!id) return;
    const res = await fetchRetry(`${API_BASE}/ideas/${id}/tasks`, { headers });
    if (!res.ok) return;
  ...
  async function loadLogs(ideaId?: string) {
    const id = ideaId || selectedIdeaId;
    if (!id) return;
    const res = await fetchRetry(`${API_BASE}/ideas/${id}/update_logs?limit=8`, { headers });
    if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    function onMouseUp() {
      ...
      void fetchRetry(`${API_BASE}/tasks/reorder`, {
        method: "PATCH",
        headers,
