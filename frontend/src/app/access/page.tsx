...
export default function SettingsPage() {
  ...
  async function loadAIStatus() {
    setAiLoading(true);
    try {
      const res = await fetchRetry(`${API_BASE}/settings/ai`, { headers });
  ...
  async function ingestReports() {
    if (!token) return;
    const ideasRes = await fetchRetry(`${API_BASE}/ideas`, { headers });
    ...
    const res = await fetchRetry(`${API_BASE}/ingest/daily_reports/bulk?idea_id=${ideaId}`, { method: "POST", headers });
